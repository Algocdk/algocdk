...
function createViewerPeerConnection() {
    ...
    // When we receive a remote track — show it in a <video> element
    peerConnection.ontrack = (event) => {
        console.log('[WebRTC] Got remote track:', event.track.kind);
        showRemoteStream(event.streams[0]);
    };

    // Send ICE candidates to admin via WebSocket
    ...
    peerConnection.onconnectionstatechange = () => {
        ...
        if (peerConnection.connectionState === 'connected') {
            ...
            showNotification('Stream connection failed. Retrying...', 'error');
            // Notify admin to resend offer
...
function showRemoteStream(stream) {
    // Replace canvas with a <video> element for WebRTC
        ...
        card.innerHTML = `
            <div class="flex items-start justify-between mb-4">
                <div class="flex items-center space-x-3">
                    <div class="w-12 h-12 rounded-full bg-gradient-to-br from-green-500 to-green-600 flex items-center justify-center"><i class="fas fa-user-tie text-white text-xl"></i></div>
                    <div><h3 class="text-lg font-semibold text-white">${session.admin_name}</h3><p class="text-sm text-slate-400">Live Trading Session</p></div>
                </div>
                <div class="flex items-center space-x-2"><div class="w-2 h-2 bg-green-500 rounded-full pulse"></div><span class="text-green-400 text-sm font-medium">LIVE</span></div>
            </div>
            <div class="space-y-2 mb-4">
                <div class="flex items-center text-sm text-slate-400"><i class="fas fa-clock mr-2 text-blue-400"></i>Started ${new Date(session.started_at).toLocaleString()}</div>
                <div class="flex items-center t"></i>WebRTC — low latency stream</div>
            </div>
            <button onclick="requestJoinSession(${session.id})" class="w-full btn-primary py-3 rounded-lg font-medium transition-all shadow-lg hover:shadow-xl">
...
async function requestJoinSession(sessionId) {
    try {
        ...
        if (response.ok) {
            const sessionsRes = await fetch(`${API_BASE}/api/screenshare/sessions`, { headers: { 'Authorization': `Bearer ${getToken()}` } });
            const sessionsData = await sessionsRes.json();
            currentSession = sessionsData.sessions?.find(s => s.id === sessionId);

            if (currentSession) {
                joinRequestStatus = data.auto_approved ? 'approved' : 'pending';
                localStorage.setItem('activeScreenShareSession', JSON.stringify({ sessionId, status: joinRequestStatus, session: currentSession }));
                showWaitingForApproval();
                connectWebSocket(sessionId);

                if (data.auto_approved) {
                    setApprovedUI();
                    // Tell admin we're ready for WebRTC
                    setTimeout(() => {
...
function showWaitingForApproval() {
    ...
    elements.waitingMessage.i"fas fa-hourglass-half text-6xl mb-4 text-yellow-500"></i><p class="text-lg text-yellow-400">Waiting for Admin Approval...</p><p class="text-sm text-slate-400 mt-2">The admin will review your request shortly</p>`;
}
...
function connectWebSocket(sessionId) {
    ...
    ws.onopen = () => console.log('Connected to session WS');
    ...
    ws.onclose = () => {
        if (joinRequestStatus === 'approved' && currentSession) {
            ...
            setTimeout(() => connectWebSocket(currentSession.id), 2000);
        }
    };
}
...
function handleWebSocketMessage(msg) {
    switch (msg.type) {
        ...
        // Fallback: legacy frame data (if server still sends it)
        case 'screen_data':
...
// Legacy fallback for canvas rendering
function displayScreenFrame(frameData) {
    elements.waitingMessage.style.display = 'none';
    elements.screenCanvas.style.display = 'block';
    const img = new Image();
    img.onload = () => {
        const ctx = elements.screenCanvas.getContext('2d');
...
function addChatMessage(message, type = 'user', username = 'You') {
    ...
    if (type === 'system') {
        ...
        div.innerHTML = `<div class="flex items-start gaplass="w-8 h-8 rounded-full ${isMe ? 'bg-gradient-to-br from-blue-500 to-blue-600' : 'bg-gradient-to-br from-purple-500 to-purple-600'} flex items-center justify-center text-xs font-semibold flex-shrink-0">${username.charAt(0).toUpperCase()}</div><div class="flex-1"><p class="text-xs ${isMe ? 'text-blue-400' : 'text-purple-400'} font-medium mb-1">${username}</p><p class="text-white text-sm break-words">${escapeHtml(message)}</p></div></div>`;
    }
    elements.chatMessages.appendChild(div);
ges.scrollTop = elements.chatMessages.scrollHeight;
}
...
function toggleMic() {
    ...
    if (audioStream) {
        audioStream.getAudioTracks().forEach(t => { t.enabled = !isMicMuted; });
    }
    updateMicButton();
...
function updateMicButton() {
    if (isMicMuted) {
        ...
        elements.micBtn.cla-green-400', 'mic-active');
    } else {
...
function initEmojiPicker() {
    emojis.forEach(emoji => {
        ...
        btn.onclick = () => { elements.chatInput.value += emoji; elements.chatInput.focus(); elements.emojiPicker.classList.add('hidden'); };
        elements.emojiPicker.appendChild(btn);
    });
}

elements.emojiBtn.addEventListener('click', (e) => { e.stopPropagation(); elements.emojiPicker.classList.toggle('hidden'); });
istener('click', (e) => { if (!elements.emojiPicker.contains(e.target) && e.target !== elements.emojiBtn) elements.emojiPicker.classList.add('hidden'); });
...
document.addEventListener('fullscreenchange', () => { if (!document.fullscreenElement) elements.fullscreenBtn.innerHTML = '<i class="fas fa-expand"></i>'; });

// ── Event listeners ──────────────────────────────────
elements.leaveBtn.addEventListener('click', leaveSession);
elements.sendChatBtn.addEventListener('click', sendChat);
elements.chatInput.addEventListener('keypress', (e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendChat(); } });
...
if (!getToken()) {
    ...
    fetch(`${API_BASE}/api/user/profile`, { headers: { 'Authorization': `Bearer ${getToken()}` } })
        .then(r => r.json()).then(d => { if (d.user?.id) localStorage.setItem('user_id', d.user.id); });
        ...
        try {
            ...
            fetch(`${API_BASE}/api/screenshare/sessions`, { headers: { 'Authorization': `Bearer ${getToken()}` } })
                .then(r => r.json()).then(data => {
                    const active = data.sessions?.find(s => s.id === sessionData.sessionId && s.is_active);
                    if (active) {
                        ...
                        if (sessionData.status === 'approved') {
                            setApprovedUI();
                            setTimeout(() => { if (ws && ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify({ type: 'viewer_ready' })); }, 500);
                        }
                    } else {
                        localStorage.removeItem('activeScreenShareSession');
                        loadActiveSessions();
          }
                }).catch(() => { localStorage.removeItem('activeScreenShareSession'); loadActiveSessions(); });
